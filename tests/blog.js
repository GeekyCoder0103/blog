...
describe("blog tests", () => {
  ...
  it("signup a new user", async () => {
    const { user, name, avatar } = await createSuper(program, provider);
  ...
  it("creates a new post", async () => {
    ...
    const { userAccount } = await createSuper(program, provider);

    const { title, post, content } = await createPostfix(
      program,
      provider,
      blogAccount,
      userAccount
